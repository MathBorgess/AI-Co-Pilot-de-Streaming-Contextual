"""Tests for AlertAgent, ActionAgent, and system latency with new agents."""
...
class TestAlertAgent:
    ...
    def test_fires_on_decision_keyword(self):
        agent = AlertAgent()
        history = agent.process(
            "We need to decide whether to build or buy the solution.")
        assert len(history) == 1
        assert history[0]["label"] == "Key Decision Detected"
        assert history[0]["emoji"] == "⚠️"

    def test_fires_on_deadline_keyword(self):
        agent = AlertAgent()
        history = agent.process(
            "The deadline is next quarter and we cannot slip further.")
        assert len(history) == 1
    ...
    def test_fires_on_conflict_keyword(self):
        agent = AlertAgent()
        history = agent.process(
            "The team is split on this approach and tension is rising.")
        assert len(history) == 1
    ...
    def test_fires_on_risk_keyword(self):
        agent = AlertAgent()
        history = agent.process(
            "Legal has flagged serious compliance concerns with the vendor.")
        assert len(history) == 1
    ...
    def test_fires_on_competitor_keyword(self):
        agent = AlertAgent()
        history = agent.process(
            "Our competitor is already gaining traction with enterprise clients.")
        assert len(history) == 1
    ...
    def test_fires_on_critical_keyword(self):
        agent = AlertAgent()
        history = agent.process(
            "This is a critical blocker for the entire roadmap.")
        assert len(history) == 1
    ...
    def test_fires_on_financial_keyword(self):
        agent = AlertAgent()
        history = agent.process(
            "The budget for this initiative needs board approval first.")
        assert len(history) == 1
    ...
    def test_no_alert_for_neutral_content(self):
        agent = AlertAgent()
        history = agent.process(
            "The meeting started at nine and the agenda was shared.")
    ...
    def test_caps_history_at_eight(self):
        ...
        for _ in range(10):
            agent.alert_history.append(
                {"emoji": "⚠️", "label": "Key Decision Detected", "detail": "x"})
        # Process one more
    ...
    def test_one_alert_per_chunk(self):
        ...
        history = agent.process(
            "This is a critical blocker and we need to decide immediately.")
        assert len(history) == 1
...
class TestActionAgent:
    def test_initial_state(self):
        agent = ActionAgent()
        assert len(agent.current_actions) == 0

    def test_generates_actions(self):
        agent = ActionAgent()
        actions = agent.process("We need to decide on the architecture today.")
        assert isinstance(actions, list)
        assert len(actions) > 0

    def test_actions_start_with_arrow(self):
        agent = ActionAgent()
        actions = agent.process("We need to decide on the launch date.")
        for a in actions:
            assert a.startswith("→"), f"Action should start with '→': {a}"

    def test_decision_context_gives_decision_actions(self):
        agent = ActionAgent()
        directions = agent.process(
            "We need to decide whether to build or buy the solution.")
        ...
        assert any(word in combined for word in [
                   "assign", "dri", "deadline", "document"])

    def test_risk_context_gives_risk_actions(self):
        agent = ActionAgent()
        directions = agent.process(
            "Legal has flagged compliance concerns with the vendor.")
        combined = " ".join(directions).lower()
        assert any(word in combined for word in [
                   "risk", "stakeholder", "mitigation", "contingency"])

    def test_deadline_context_gives_deadline_actions(self):
        agent = ActionAgent()
        directions = agent.process(
            "The deadline is next quarter and we cannot slip.")
        combined = " ".join(directions).lower()
        assert any(word in combined for word in [
                   "dependencies", "milestone", "scope", "deadline"])

    def test_conflict_context_gives_conflict_actions(self):
        agent = ActionAgent()
        directions = agent.process(
            "The team is split and tension is high - both sides are pushing back hard.")
        combined = " ".join(directions).lower()
        assert any(word in combined for word in [
                   "document", "perspectives", "escalate", "spike"])

    def test_returns_actions_for_neutral_content(self):
        agent = ActionAgent()
        directions = agent.process(
            "The meeting started and everyone introduced themselves.")
        assert isinstance(directions, list)
        # always returns something (default category)
        assert len(directions) > 0

    def test_accepts_recent_alerts_parameter(self):
        agent = ActionAgent()
        alerts = [
            {"emoji": "⚠️", "label": "Key Decision Detected", "detail": "..."}]
        directions = agent.process(
            "We need to decide on the vendor.", recent_alerts=alerts)
        assert isinstance(directions, list)
    ...
    def test_max_three_actions(self):
        agent = ActionAgent()
        directions = agent.process(
            "We need to decide on the build vs buy question.")
...
class TestNewAgentsLatency:
    ...
    def test_orchestrator_result_has_all_fields(self):
        orch = Orchestrator()
        result = orch.process(
            "We need to decide on the build vs buy strategy today.", chunk_index=1, total_chunks=5)
        required = {"summary", "questions", "insights", "alerts", "actions", "directions",
                    "chunkIndex", "totalChunks", "processingTimeMs", "totalChunksProcessed", "focusTerms"}
